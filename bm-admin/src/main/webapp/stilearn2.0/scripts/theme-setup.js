$(function(){
    ...
    // set default menu active state add by HeMingwei 为
    // add by HeMingwei 业务url尽量要在跟目录之后大于等于一个单词 以便可以title正常可用
    var setActiveMenu = function(){
        var lastIndexUrl = document.location.pathname,
            urlFilename = (lastIndexUrl.substr(lastIndexUrl.lastIndexOf("/")-9,lastIndexUrl.length-(lastIndexUrl.lastIndexOf("/")+2)) == '') ? '/index.html' : lastIndexUrl,
        ...
        if (label == '首页') {
            ...
            $('.content-title').html('<i class="' + icon.attr('class') + '"></i> 欢迎登陆石狮青少年中心手机报名后台管理系统!');
            $(".breadcrumb li:eq(0)").text("");
            $(".breadcrumb li:eq(1)").remove();
        } 
        else{
            // Use label for content title
            $('.content-title').html('<i class="' + icon.attr('class') + '"></i> ' + label);
            var par = activeMenuUl.parent().children('a');
            $(".breadcrumb li:eq(0)").html("<a style='text-decoration:none;'><i class='" + icon.attr('class') + "'></i> "+ par.last().text()+"</a>");
            if($(".breadcrumb li:eq(1)").length > 0){
            	$(".breadcrumb li:eq(1)").text(label);
            }else{
            	$(".breadcrumb li:eq(0)").parent().append("<li class='active'> "+label+"</li>");
            }
            
        }

        // change page title
        $('title').text(label + ' | XinyinTech');
    }
    ...
    var host = window.location.protocol + '//' + window.location.host + '',
        ...
        reExecuteScripts = [
            '/stilearn2.0/scripts/google-code-prettify/run_prettify.js', 
            '/stilearn2.0/scripts/bootstrap-setup.js', 
            '/stilearn2.0/scripts/jqueryui-setup.js', 
            '/stilearn2.0/scripts/dependencies-setup.js', 
            '/stilearn2.0/scripts/demo-setup.js',
            '/stilearn2.0/scripts/custom/common.js'
    ...
    $(document).on('shown.bs.modal', '.modal', function (e) {
        ...
        if (data_sound != 'off') {
            ...
            if (data_sound == 'hello') {
                $.playSound(projectRootPath+'/stilearn2.0/sounds/' + data_sound);
            }
