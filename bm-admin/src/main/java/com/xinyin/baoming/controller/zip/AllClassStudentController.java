 ...
 **/
@Controller
@RequestMapping(value = "/zip/allclassstudent")
public class AllClassStudentController extends BaseController {

	private static final Logger logger = Logger.getLogger(AllClassStudentController.class);

	@Autowired
	IAllClassStudentService iAllClassStudentService;
	...
	@RequestMapping()
	public String init() {
		return Views.ZIP_ALLCLASSSTUDENT;
	}

	/**
	 * 
	 * 班级学生关联归档表信息(返回Json数据)
	 * 
	 * @author HeMingwei
	 * @param
	 * @return
	...
	@ResponseBody
	public Object[] queryAllClassStudentJsonData(TzAllClassStudent record,
		...
		try {
			...
			PageList<TzAllClassStudent> list = null;
			...
			list = iAllClassStudentService.selectBySelective(record);
	...
	@ResponseBody
	public String save(HttpSession session, TzAllClassStudent record) {
		...
		try{
			...
			resMsg = iAllClassStudentService.insertSelective(record);
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
	...
	@ResponseBody
	public TzAllClassStudent queryById(@PathVariable("id") String id) {
		TzAllClassStudent record = null;
		try{
			record = iAllClassStudentService.selectByPrimaryKey(id);
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
		}
		return record;
	...
	@ResponseBody
	public String modify(HttpSession session, TzAllClassStudent record) {
		String resMsg = StringUtils.EMPTY;
		try{
			TsUser user = (TsUser) session.getAttribute(Constant.LOGIN_KEY);
			record.setUpdateBy(user.getUsername());
			resMsg = iAllClassStudentService.updateByPrimaryKeySelective(record);
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
			return Constant.SYSTEM_EXCEPTION;
		}
	...
	public String delete(@RequestParam(value="id") String id) {
		String resMsg = StringUtils.EMPTY;
		try{
			resMsg = iAllClassStudentService.deleteByPrimaryKey(id);
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
			return Constant.SYSTEM_EXCEPTION;
		}
