 ...
 **/
@Controller
@RequestMapping(value = "/operation/teacher")
public class TeacherController extends BaseController {
	...
	public String init(Model model) {
		...
		return Views.OPERATION_TEACHER;
	}
	...
	@ResponseBody
	public Object[] queryTeacherJsonData(TmTeacher record,
			...
			@RequestParam(value="fcol[0]",required = false) String id, @RequestParam(value="fcol[1]",required = false) String idcardNo,
			@RequestParam(value="fcol[2]",required = false) String phonenumber, @RequestParam(value="fcol[3]",required = false) String name,
			@RequestParam(value="fcol[4]",required = false) String sex, @RequestParam(value="fcol[5]",required = false) String pictureUrl,
		...
		try {
			...
			map.put("IDCARD_NO", sort1);
			map.put("PHONENUMBER", sort2);
			map.put("NAME", sort3);
			map.put("SEX", sort4);
			map.put("PICTURE_URL", sort5);
			...
			list = iTeacherService.selectBySelective(record);
			Object[][] objs = new Object[list.size()][11]; // 数组长度需要与前端列数保持一致
			for(int i = 0;i<list.size();i++){
				objs[i][0] = Tools.rowSelectionSet(list.get(i).getId());
				objs[i][1] = Tools.rowSelectionSet(list.get(i).getIdcardNo());
				objs[i][2] = Tools.rowSelectionSet(list.get(i).getPhonenumber());
				objs[i][3] = Tools.rowSelectionSet(list.get(i).getName());
				objs[i][4] = Tools.rowSelectionSet(list.get(i).getSex());
				objs[i][5] = Tools.rowSelectionSet(list.get(i).getPictureUrl());
	...
	@ResponseBody
	public String save(HttpSession session, TmTeacher record) {
		...
		try{
			...
			resMsg = iTeacherService.insertSelective(record);
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
	...
	@ResponseBody
	public TmTeacher queryById(@PathVariable("id") String id) {
		TmTeacher record = null;
		try{
			record = iTeacherService.selectByPrimaryKey(id);
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
		}
		return record;
	...
	@ResponseBody
	public String modify(HttpSession session, TmTeacher record) {
		String resMsg = StringUtils.EMPTY;
		try{
			TsUser user = (TsUser) session.getAttribute(Constant.LOGIN_KEY);
			record.setUpdateBy(user.getUsername());
			resMsg = iTeacherService.updateByPrimaryKeySelective(record);
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
			return Constant.SYSTEM_EXCEPTION;
		}
	...
	public String delete(@RequestParam(value="id") String id) {
		String resMsg = StringUtils.EMPTY;
		try{
			resMsg = iTeacherService.deleteByPrimaryKey(id);
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
			return Constant.SYSTEM_EXCEPTION;
		}
	...
	@ResponseBody
	public String authorization(HttpSession session, @RequestParam(value="id") String id) {
		String resMsg = StringUtils.EMPTY;
		try{
			TsUser user = (TsUser) session.getAttribute(Constant.LOGIN_KEY);
			resMsg = iTeacherService.authorization(id, user.getUsername());
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
