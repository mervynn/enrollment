...
public class ShoppingCartController extends BaseController {
	...
	@RequestMapping()
	public String init() {
		return Views.USER_SHOPPINGCART;
	}
	...
	@ResponseBody
	public Object[] queryShoppingCartJsonData(TtShoppingCart record,
			...
			@RequestParam(value="fcol[2]",required = false) String classId, @RequestParam(value="fcol[3]",required = false) String operationTeacherId,
		...
		try {
			...
			map.put("OPERATION_TEACHER_ID", sort3);
			...
			PageList<TtShoppingCart> list = null;
			...
			for(int i = 0;i<list.size();i++){
				...
				objs[i][3] = Tools.rowSelectionSet(list.get(i).getOperationTeacherId());
	...
	@ResponseBody
	public String save(HttpSession session, TtShoppingCart record) {
		...
		try{
			...
			resMsg = iShoppingCartService.insertSelective(record);
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
	...
	@ResponseBody
	public TtShoppingCart queryById(@PathVariable("id") String id) {
		TtShoppingCart record = null;
		try{
			record = iShoppingCartService.selectByPrimaryKey(id);
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
		}
		return record;
	...
	@ResponseBody
	public String modify(HttpSession session, TtShoppingCart record) {
		String resMsg = StringUtils.EMPTY;
		try{
			TsUser user = (TsUser) session.getAttribute(Constant.LOGIN_KEY);
			record.setUpdateBy(user.getUsername());
			resMsg = iShoppingCartService.updateByPrimaryKeySelective(record);
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
			return Constant.SYSTEM_EXCEPTION;
		}
	...
	public String delete(@RequestParam(value="id") String id) {
		String resMsg = StringUtils.EMPTY;
		try{
			resMsg = iShoppingCartService.deleteByPrimaryKey(id);
		}catch(Exception e){
