 ...
 **/
@Controller
@RequestMapping(value = "/zip/allclass")
public class AllClassController extends BaseController {

	private static final Logger logger = Logger.getLogger(AllClassController.class);

	@Autowired
	IAllClassService iAllClassService;
	...
	@ResponseBody
	public Object[] queryAllClassJsonData(TzAllClass record,
		...
		try {
			...
			list = iAllClassService.selectBySelective(record);
	...
	@ResponseBody
	public String save(HttpSession session, TzAllClass record) {
		...
		try{
			...
			resMsg = iAllClassService.insertSelective(record);
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
	...
	@ResponseBody
	public TzAllClass queryById(@PathVariable("id") String id) {
		TzAllClass record = null;
		try{
			record = iAllClassService.selectByPrimaryKey(id);
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
		}
		return record;
	...
	@ResponseBody
	public String modify(HttpSession session, TzAllClass record) {
		String resMsg = StringUtils.EMPTY;
		try{
			TsUser user = (TsUser) session.getAttribute(Constant.LOGIN_KEY);
			record.setUpdateBy(user.getUsername());
			resMsg = iAllClassService.updateByPrimaryKeySelective(record);
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
			return Constant.SYSTEM_EXCEPTION;
		}
	...
	public String delete(@RequestParam(value="id") String id) {
		String resMsg = StringUtils.EMPTY;
		try{
			resMsg = iAllClassService.deleteByPrimaryKey(id);
		}catch(Exception e){
			logger.error(Constant.SYSTEM_EXCEPTION,e);
			return Constant.SYSTEM_EXCEPTION;
		}
